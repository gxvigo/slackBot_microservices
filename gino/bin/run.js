...
slackClient.addAuthenticatedHandler(rtm, () => {   // ()=>{}  new syntax for function in ES6
    ...
    server = service.listen(3000, function () {
        console.log("'gino' application listening in port 3000");  // 'pro' logging modules are: node-bunyan or winston
    })

    // Another option to execute statements is creating an event listener
    server.on('listening', function () {
        console.log(`[event] 'gino' application listening on ${server.address().port} in ${service.get('env')} mode`);
